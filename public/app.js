...
async function refreshFunnel() {
  ...
  $('fAssigned').textContent = f.assigned;
  $('fCalled').textContent = f.called;
  $('fReached').textContent = f.reached;
  $('fDissatisfied').textContent = f.dissatisfied;
  $('fResolved').textContent = f.resolved;
}
...
async function refreshAll() {
  await refreshTopbar();
  await Promise.all([refreshMetrics(), refreshFunnel(), refreshBreakdown(), refreshCallLog()]);
}
